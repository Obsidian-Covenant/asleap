...
int getnextrec(struct hashpass_rec *rec, FILE * fp, int fillpass)
{
    ...
    if (fillpass) {
        ...
        if (fread(rec->password, passlen, 1, fp) != 1) {
            perror("[getnextrec] fread(password)");
            free(rec->password);
            rec->password = NULL;
            return -1;
        }
        ...
        if (fseek(fp, passlen, SEEK_CUR) != 0) {
            perror("[getnextrec] fseek");
            return -1;
        }
    }

    if (fread(rec->hash, 16, 1, fp) != 1) {
        perror("[getnextrec] fread(hash)");
        return -1;
    }
...
int main(int argc, char *argv[])
{
    ...
    while (fgets(password, MAX_NT_PASSWORD + 1, inputfl) != NULL) {
        ...
        /* Accommodate Windows-style CRLF */
 ...
 */
        ...
        for (brecsub = 0; brecsub < bucketrecords; brecsub++) {
            ...
            if (fread(brec[brecsub].password, passlen, 1,
                      hbucket[hsub].sbucket) != 1) {
                fprintf(stderr,
                    "Unable to read password from bucket %02x. ", hsub);
                perror("fread");
                closebuckets(hbucket, 256);
                exit(-1);
            }
            ...
            if (fread(brec[brecsub].hash, 16, 1,
                      hbucket[hsub].sbucket) != 1) {
                fprintf(stderr,
                    "Unable to read hash from bucket %02x. ", hsub);
                perror("fread");
                closebuckets(hbucket, 256);
                exit(-1);
            }
