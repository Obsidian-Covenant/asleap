...
int getmschapbrute(struct asleap_data *asleap_ptr)
{
    ...
    while (fgets(password, MAX_NT_PASSWORD + 1, wordlist) != NULL) {
        ...
        /* optional: strip CR if present */
        if (len > 0 && password[len - 1] == '\r') {
            password[--len] = '\0';
        }

        NtPasswordHash(password, len, pwhash);
    ...
    if (IsBlank(asleap_ptr->dictidx)) {
        ...
        for (;;) {
            ...
            if (fread(&rec.rec_size, sizeof(rec.rec_size), 1, buffp) != 1) {
                /* EOF or error - either way, stop scanning */
                if (!feof(buffp)) {
                    perror("[getmschappw] fread(rec_size)");
                }
                break;
            }
            ...
            if (passlen <= 0 || passlen > MAX_NT_PASSWORD) {
                fprintf(stderr,
                    "[getmschappw] Invalid password length %d.\n",
                    passlen);
                break;
            }

            if (fread(&password_buf, passlen, 1, buffp) != 1) {
                perror("[getmschappw] fread(password)");
                break;
            }

            if (fread(&zpwhash, 16, 1, buffp) != 1) {
                perror("[getmschappw] fread(hash)");
                break;
            }
